...
serve(async (req) => {
  ...
  try {
    ...
    if (timeframe === '1D') {
      ...
      console.log('Raw intraday data received:', data.slice(0, 2), '... and', data.length - 2, 'more items');
      ...
      const chartData = data
        .filter(item => {
          ...
          const isToday = itemDate.toISOString().split('T')[0] === today;
          if (isToday) {
            console.log('Found data point for today:', item.date, item.close);
          }
          return isToday;
        })
        .map(item => ({
      ...
      console.log('Sample of processed data:', chartData.slice(0, 2));
      ...
      if (chartData.length === 0) {
        ...
        if (Array.isArray(quoteData) && quoteData.length > 0) {
          console.log('Using quote data as fallback:', quoteData[0].price);
          return new Response(
            JSON.stringify([{
